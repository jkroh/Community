...
//	var html_str = "<div class='cal_border' id='"+box_date+"'>"+va_type+"<br/>"+view_date+"</div>";
	...
	if(va_type == "year"){
		if(year_in_count.value == 0){	//연차 시작일 입력
			...
			value_anther_va = chk_another_va(view_date, 'y_start', page_year);
			...
			value_anther_va = chk_another_va(view_date, 'y_end', page_year);
			if(value_anther_va == 1){
				alert("다른연차와 기간이 겹칩니다.");
				return;	
			}
		...
		if(half_in_count.value == 0){	
			...
			value_anther_va = chk_another_va(view_date, 'h_start', page_year);
			if(value_anther_va == 1){
				alert("다른연차와 기간이 겹칩니다.");
				return;	
			}
			...
			value_anther_va = chk_another_va(view_date, 'h_end', page_year);
			if(value_anther_va == 1){
				alert("다른연차와 기간이 겹칩니다.");
				return;	
			}
		...
		if(public_in_count.value == 0){		//공가 시작일 입력
			...
			value_anther_va = chk_another_va(view_date, 'p_start', page_year);
			if(value_anther_va == 1){
				alert("다른연차와 기간이 겹칩니다.");
				return;	
			}
			...
			value_anther_va = chk_another_va(view_date, 'p_end', page_year);
			if(value_anther_va == 1){
				alert("다른연차와 기간이 겹칩니다.");
				return;	
			}
...
function chk_another_va(chk_val, type, chk_year){
	...
	var y_start_year = document.getElementById('y_start_year').value;
	var y_end_year = document.getElementById('y_end_year').value;
	var h_start_year  = document.getElementById('h_start_year').value;
	var h_end_year  = document.getElementById('h_end_year').value;
	var p_start_year = document.getElementById('p_start_year').value;
	var p_end_year = document.getElementById('p_end_year').value;
	...
	chk_val = integer_string(chk_val,chk_year);
	year_start = integer_string(year_start,y_start_year);
	year_end = integer_string(year_end,y_end_year);
	half_start = integer_string(half_start,h_start_year);
	half_end = integer_string(half_end,h_end_year);
	public_start = integer_string(public_start,p_start_year);
	public_end = integer_string(public_end,p_end_year);




	if(type == 'y_start'){
		if(((chk_val > half_end || chk_val < half_start) && !(year_end > half_end && chk_val < half_start)) && ((chk_val < public_start || chk_val > public_end ) && !(chk_val < public_start && year_end > public_end ))){
			result_val = 0;	
		}
	}else if(type == 'y_end'){
		if(((chk_val > half_end || chk_val < half_start) && !(chk_val > half_end && year_start < half_start)) && ((chk_val < public_start || chk_val > public_end ) && !(year_start < public_start && chk_val > public_end ))){
			result_val = 0;	
		}
	}else if(type == 'h_start'){
		if(((chk_val > year_end || chk_val < year_start) && !(half_end > year_end && chk_val < year_start)) && ((chk_val < public_start || chk_val > public_end ) && !(chk_val < public_start && half_end > public_end ))){
			result_val = 0;	
		}
	}else if(type == 'h_end'){
		if(((chk_val > year_end || chk_val < year_start) && !(chk_val > year_end && half_start < year_start)) && ((chk_val < public_start || chk_val > public_end ) && !(half_start < public_start && chk_val > public_end ))){
			result_val = 0;	
		}
	}else if(type == 'p_start'){
		if(((chk_val > year_end || chk_val < year_start) && !(public_end > year_end && chk_val < year_start)) && ((chk_val < half_start || chk_val > half_end ) && !(chk_val < half_start && public_end > half_end ))){
			result_val = 0;	
		}
	}else{
		if(((chk_val > year_end || chk_val < year_start) && !(chk_val > year_end && public_start < year_start)) && ((chk_val < half_start || chk_val > half_end ) && !(public_start < half_start && chk_val > half_end ))){
			result_val = 0;	
		}
	}
	return result_val;
}
function integer_string(string_data,year){
	var string_data_m = string_data.substring(0,2);
	var string_data_d = string_data.substring(3,5);
	
	string_data = year+string_data_m+string_data_d;
